...
class Dataset(minnt.TransformedDataset):
    def transform(self, image, label):
        image = torchvision.transforms.v2.functional.to_image(image)
        image = image.squeeze(0).to(torch.int32)
        return image, label
...
def main(args: argparse.Namespace) -> None:
    ...
    def configure(model, logdir):
        model.configure(
            optimizer=(optimizer := minnt.optimizers.LazyAdam(model, args.learning_rate)),
    ...
    loaded_model = Model(args)
    configure(loaded_model, logdir=None)
    loaded_model.load_weights(f"{model.logdir}/model.pt", "optimizer.pt")

    for m in [model, loaded_model]:
        with torch.random.fork_rng():
            m.fit(train, dev=dev, epochs=1)
            m.evaluate(test)
